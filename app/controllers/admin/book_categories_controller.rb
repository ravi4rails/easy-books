class Admin::BookCategoriesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @book_category.update(book_category_params)
        format.html { redirect_to admin_book_category_path(@book_category), notice: 'Book category was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_book_categories_url, notice: 'Book category was successfully destroyed.' }
